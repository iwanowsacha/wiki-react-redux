...
export function TagList(props: TagListProps) {
    ...
    return(
        <ModalContainer className="border-2" title="All tags">
            ...
            <div className={`grid p-2 ${props.className}`}>
                {isSearching
                    ? filteredTags.map((f: string) => <TagPill onTagClick={props.onTagClick} isDraggable={true} isSelected={selectedTags.includes(f.toLowerCase())} key={f} title={f} />)
                    : tags.map((t: string) => <TagPill onTagClick={props.onTagClick} isDraggable={true} isSelected={selectedTags.includes(t.toLowerCase())} key={t} title={t} />)
                }
            </div>
        </ModalContainer>
    );
