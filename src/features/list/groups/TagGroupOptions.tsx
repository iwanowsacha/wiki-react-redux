...
import { addGroup, removeGroup, selectAllGroups, updateGroup } from "./groupsSlice";
import { Droppable } from "../../../components/Droppable";
import { ModalContainer } from "../../../components/ModalContainer";
import { TextInput } from "../../../components/ControlledTextInput";

export function TagGroupOptions() {
    ...
    const handleTagRemoveDrop = (e: SyntheticEvent) => {
    ...
    const handleSelectedRemoveGroupChange = (e: SyntheticEvent) => {
        ...
        <ModalContainer className="border-2" title="Group options">
            <div className="p-2">
                ...
                <div className="m-2 flex bg-primary p-2 text-secondary">
                    <TextInput text={textAddGroup} onTextChange={handleTextAddGroupChange} color="bg-primary" />
                    <button onClick={handleAddGroupButtonClick}
                ...
                <div className="m-2 grid grid-col-1 md:grid-cols-4 bg-primary text-secondary p-2">
                    ...
                    <select className="bg-primary h-8 w-full col-span-4 md:col-span-3" placeholder="Remove group"
                    value={selectedRemoveGroup} onChange={handleSelectedRemoveGroupChange}>
                        {
                            tagGroups.map((g) => <option key={g.title} value={g.title}>{g.title}</option>)
                        }
                    </select>
                    <button onClick={handleRemoveGroupButtonClick} className="col-span-4 md:col-span-1 text-base lg:text-2xl material-icons self-center justify-self-center lg:justify-self-end pl-1 lg:border-l-2 border-secondary text-red-500">
                        delete
                    </button>
                </div>
