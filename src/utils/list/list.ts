...
import { DIRECTORIES, createDirectoryIfNotExists, renameDirectory } from '../../directories';
import path from 'path';
import * as fs from 'fs-extra';
import { ipcMain } from 'electron';

export const saveList = async (list: List, newTitle: string, listImages: ListItemImageChanges) => {
    ...
    if (previousTitle !== newTitle) {
        await renameDirectory(path.join(DIRECTORIES.lists, previousTitle), path.join(DIRECTORIES.lists, newTitle));
    } else {
        await createDirectoryIfNotExists(path.join(DIRECTORIES.lists, list.title, 'images'));
    }
...
const listImagesRename = (images: {[key: string]: string}, list: List) => {
    const rename = Object.entries(images).map(([key, value]) => {
        ...
        return renameDirectory(path.join(DIRECTORIES.lists, list.title, 'images', value), path.join(DIRECTORIES.lists, list.title, 'images', key + path.extname(value)));
      });
    return rename;
}
